...
chol_inv_jitter_optim <- function(mat, pen_diag, max_attempts = 5) {
  ...
  stop("Impossible de calculer l'inverse après ", max_attempts, " tentatives.")
}
chol_inv_jitter_optim_compiled <- cmpfun(chol_inv_jitter_optim)


dmnorm_optim <- function(x, mu, inv_Sigma, log = FALSE) {
  if (is.vector(x)) x <- matrix(x, nrow = 1)
  n <- nrow(x)
  p <- ncol(x)
  if (is.vector(mu)) {
    if (length(mu) != p) stop("The length of mu must match the number of columns in x.")
    mu <- matrix(rep(mu, n), ncol = p, byrow = TRUE)
  } else if (is.matrix(mu)) {
    if (ncol(mu) != p || nrow(mu) != n) stop("The dimensions of mu must match the dimensions of x.")
  } else {
    stop("mu must be a vector or a matrix.")
  }
  z <- x - mu
  ...
  logdetS <- -determinant(inv_Sigma, logarithm = TRUE)$modulus
  ssq <- as.vector(rowSums((z %*% inv_Sigma) * z))
  loglik <- -(n * log(2 * pi) + logdetS + ssq) / 2
  if (log) loglik else exp(loglik)
}
dmnorm_optim_compiled <- cmpfun(dmnorm_optim)

sum_logGaussian_optim <- function(hp, db, mean, kern, post_cov, pen_diag) {
  kern <- set_hyperparameters(kern, hp)
  ...
  cov <- cov + post_cov
  inv <- chol_inv_jitter_optim_compiled(cov, pen_diag = pen_diag)
  if (length(mean) == 1) {
    mean_vec <- rep(mean, n)
  } else {
    mean_vec <- mean
    if (length(mean_vec) != n) stop("La longueur de 'mean' doit correspondre au nombre de lignes de db$Input")
  }
  log_likelihoods <- dmnorm_optim_compiled(db$Output, mean_vec, inv, log = TRUE)
  neg_sum_log_likelihood <- -sum(log_likelihoods)
  if (is(kern, "AdditiveKernel")) {
    hps <- gt_HPs(kern)
    variance_params <- hps[grepl("variance", names(hps))]
  ...
  neg_sum_log_likelihood
}
sum_logGaussian_optim_compiled <- cmpfun(sum_logGaussian_optim)

gr_sum_logGaussian_optim <- function(hp, db, mean, kern, post_cov, pen_diag) {
  kern <- set_hyperparameters(kern, hp)
  list_hp <- get_hyperparameter_names(kern)
  output <- db$Output
  input <- as.matrix(db$Input)
...
gr_sum_logGaussian_optim_compiled <- cmpfun(gr_sum_logGaussian_optim)


optim_hp_L_BFGS_B <- function(hp, db, mean, kern, post_cov, pen_diag=1e-6, verbose = FALSE, max_iter = 100) {
  objective <- function(hp) {
    sum_logGaussian(hp, db, mean, kern, post_cov, pen_diag)
  }
  gradient <- function(hp) {
...
optim_hp_L_BFGS_B_optim <- function(hp, db, mean, kern, post_cov, pen_diag=1e-6, verbose = FALSE, max_iter = 100) {
  ...
  gradient <- function(hp) {
    gr_sum_logGaussian_optim_compiled(hp, db, mean, kern, post_cov, pen_diag)
  }
  lower <- rep(1e-6, length(hp))
  names(lower) <- names(hp)
  upper <- rep(1e6, length(hp))
...
# Benchmark
mb <- microbenchmark(
  original = optim_hp_L_BFGS_B(hp, data, mean_nul, SEKernel, post_cov_nul, pen_diag=1e-6),
  optimised = optim_hp_L_BFGS_B_optim(hp, data, mean_nul, SEKernel, post_cov_nul, pen_diag=1e-6),
  times = 5
)
